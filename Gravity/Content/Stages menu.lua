...
local function movlis( event )
    if event.phase == "moved" then
      if not backanimation then
        tpos = pos + ( event.x - event.xStart );
        if tpos < 150 and tpos > -( #Stages_menu.stages * 250 ) - 150 then
          for i = 1, #Stages_menu.stages do
            Stages_menu.stages[i].x = tpos;
          end;
        end;
      end;
    ...
    elseif event.phase == "ended" or event.phase == "cancelled" then
      if tpos > 0 then
        for i = 1, #Stages_menu.stages do
          transition.to( Stages_menu.stages[i], { time = 230, x = 0 } );
        end;
        backanimation = true;
        timer.performWithDelay( 230, function() backanimation = false end );
        pos = 0;

      elseif tpos < -( #Stages_menu.stages * 250 ) then
        for i = 1, #Stages_menu.stages do
          transition.to( Stages_menu.stages[i], { time = 180, x = -( #Stages_menu.stages * 250 ) } );
        end;
        backanimation = true;
        timer.performWithDelay( 180, function() backanimation = false end );
        pos = -( #Stages_menu.stages * 250 );
